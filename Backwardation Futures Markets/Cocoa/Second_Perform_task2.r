...
for(i in 4375:nrow(data)){
  ...
  for(j in 1:6){
    ...
    listofdfs.temp[[j]] = listofdfs.temp[[j]][listofdfs.temp[[j]]$V3<=1050,]  
  }
  ...
  if(is.integer0(CL0.values)){CL0.values=NA}else{CL0.values=CL0.values}
  if(is.integer0(CL1.values)){CL1.values=NA}else{CL1.values=CL1.values}
  if(is.integer0(CL2.values)){CL2.values=NA}else{CL2.values=CL2.values}
  if(is.integer0(CL3.values)){CL3.values=NA}else{CL3.values=CL3.values}
  if(is.integer0(CL4.values)){CL4.values=NA}else{CL4.values=CL4.values}
  if(is.integer0(CL5.values)){CL5.values=NA}else{CL5.values=CL5.values}
...
write.csv(df,"Backwardation_for_Coco.csv")
