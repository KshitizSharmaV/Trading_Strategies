...
for(i in 1:nrow(data)){
  ...
  if(!identical(CL0.t,previous)){
    ...
    if(paste(listofdfs[[6]]$V1[1],".b01",sep="") != CL5.t){
      listofdfs[[6]]=read.table(paste("./b01Files/",CL5.t,sep=""),header=FALSE)
    }
  }
  ...
  for(j in 1:6){
    ...
    listofdfs.temp[[j]] = listofdfs.temp[[j]][listofdfs.temp[[j]]$V3<=1300,]  
  }
  ...
  if(is.integer0(CL0.values)){CL0.values=NA}else{CL0.values=CL0.values/100000}
  if(is.integer0(CL1.values)){CL1.values=NA}else{CL1.values=CL1.values/100000}
  if(is.integer0(CL2.values)){CL2.values=NA}else{CL2.values=CL2.values/100000}
  if(is.integer0(CL3.values)){CL3.values=NA}else{CL3.values=CL3.values/100000}
  if(is.integer0(CL4.values)){CL4.values=NA}else{CL4.values=CL4.values/100000}
  if(is.integer0(CL5.values)){CL5.values=NA}else{CL5.values=CL5.values/100000}
...
write.csv(df,"Backwardation_for_Live_Cattle2.csv")
