...
previous=paste("F.US.",each.row$Lead.Contract,sep="")
listofdfs <- list()
...
for(i in 1:nrow(data)){
  ...
  for(j in 1:6){
    ...
    listofdfs.temp[[j]] = listofdfs.temp[[j]][listofdfs.temp[[j]]$V3<=1230,]  
  }
  ...
  if(is.integer0(CL5.values)){CL5.values=NA}else{CL5.values=CL5.values/100}
...
write.csv(df,"Backwardation_for_Coffee.csv")
