...
df = data.frame(Date=integer(),Lead.Contract=character(),CL0=character(),CL1=character(),CL2=character(),CL3=character(),CL0.value=double(),CL1.value=double(),CL2.value=double(),CL3.value=double(),CL4.value=double())
...
for(i in 5075:nrow(data)){
  ...
  if(!identical(CL0.t,previous)){
    ...
    listofdfs[[5]]=read.table(paste("./b01Files/",CL4.t,sep=""),header=FALSE,fill=TRUE)
    ...
    if(paste(listofdfs[[5]]$V1[1],".b01",sep="") != CL4.t){
      listofdfs[[5]]=read.table(paste("./b01Files/",CL4.t,sep=""),header=FALSE)
    }
  }
  ...
  if(is.integer0(CL0.values)){CL0.values=NA}else{CL0.values=CL0.values/100}
  if(is.integer0(CL1.values)){CL1.values=NA}else{CL1.values=CL1.values/100}
  if(is.integer0(CL2.values)){CL2.values=NA}else{CL2.values=CL2.values/100}
  if(is.integer0(CL3.values)){CL3.values=NA}else{CL3.values=CL3.values/100}
  if(is.integer0(CL4.values)){CL4.values=NA}else{CL4.values=CL4.values/100}
...
write.csv(df,"Backwardation_for_Sugar.csv")
