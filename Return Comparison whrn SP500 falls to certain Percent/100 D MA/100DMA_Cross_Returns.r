...
# Get Closing Prices and 100 Day MA
closing.prices = data$Close
MA.100 = data$X100.D.MA.Price


# Testing strategy 
# Strategy - Check when the closing price crosses 100 Day MA on day (x+1) downward
# And on day (x) S&P500 is above the MA

# Storing length in l 
l = length(closing.prices)

# Data Frame for storing the closing days when S&P breaks MA 100
...
write.csv(data.base,"Breaking100dmovingaverage.csv")
...
different.days.gaps.returns = function(days){
  ...
  temp = data.frame(Date=dates[101],Close.price=closing.prices[101],Return=0)
  df = rbind(df,temp)
  i=101+days
  while(i < l){
...
write.csv(Levene_test,"100SD.csv")
